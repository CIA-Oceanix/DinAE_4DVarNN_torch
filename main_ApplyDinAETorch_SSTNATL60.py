        ...
        if flagProcess[kk] == 0:        
            if flagDataset == 0: ## MNIST
              ...
              if 1*0 :
                  ...
                  fileTr.append('./data/Data/DataNATL60/NATL60withMETOPMask/patchDataset_NATL60_Osmosis_METOPMask_Anomaly_20080601_20080630_Patch_128_512_005.nc')
                  fileTr.append('./data/Data/DataNATL60/NATL60withMETOPMask/patchDataset_NATL60_Osmosis_METOPMask_Anomaly_20080801_20080831_Patch_128_512_005.nc')

                  fileTt.append('./data/Data/DataNATL60/NATL60withMETOPMask/patchDataset_NATL60_Osmosis_METOPMask_Anomaly_20080701_20080731_Patch_128_512_005.nc')
              if 1*0:
...
#                x_test_missing  = x_test  * mask_test
            ...
            if flagAEType == 0: ## MLP-AE
              ...
              class Encoder(torch.nn.Module):
                  def __init__(self):
                      ...
                      self.conv1   = dinAE.ConstrainedConv2d(shapeData[0],NbFilter,(WFilter,WFilter),padding=int(WFilter/2),bias=False)                      
                      self.conv2   = torch.nn.Conv2d(NbFilter,DimAE,(1,1),padding=0,bias=False)
                      self.resnet1 = dinAE.ResNetConv2D(NbResUnit,DimAE,5,1,0)
                      self.conv1Tr = torch.nn.ConvTranspose2d(DimAE,DimAE,(4,4),stride=(4,4),bias=False)
                      self.resnet2 = dinAE.ResNetConv2D(NbResUnit,DimAE,5,1,0)
              ...
              class Encoder(torch.nn.Module):
                  def __init__(self):
                      super(Encoder, self).__init__()
                      self.pool11   = torch.nn.AvgPool2d((4,4))
                      self.conv11   = dinAE.ConstrainedConv2d(shapeData[0],NbFilter,(WFilter,WFilter),padding=int(WFilter/2),bias=False)                      
                      self.conv12   = torch.nn.Conv2d(NbFilter,DimAE,(1,1),padding=0,bias=False)
                      self.resnet1  = dinAE.ResNetConv2D(NbResUnit,DimAE,5,1,0)
                      self.conv11Tr = torch.nn.ConvTranspose2d(DimAE,DimAE,(4,4),stride=(4,4),bias=False)

                      self.conv21   = dinAE.ConstrainedConv2d(shapeData[0],NbFilter,(WFilter,WFilter),padding=int(WFilter/2),bias=False)                      
                      self.conv22   = torch.nn.Conv2d(NbFilter,DimAE,(1,1),padding=0,bias=False)
                      self.resnet2  = dinAE.ResNetConv2D(NbResUnit,DimAE,5,1,0)

                      self.convF1   = torch.nn.Conv2d(DimAE,2*DimAE,(1,1),padding=0,bias=False)
                      self.convF2   = torch.nn.Conv2d(2*DimAE,shapeData[0],(1,1),padding=0,bias=False)
                  ...
                  def forward(self, x):
                      ...
                      xlr   = self.pool11( x )
                      xlr   = self.conv11( xlr )
                      xlr   = self.conv12( F.relu( xlr ) )
                      xlr   = self.resnet1( xlr )
                      xlr   = self.conv11Tr( xlr )
                      
                      # first-scale component
                      xhr = self.conv21(x)
                      xhr = self.conv22( F.relu( xhr ) )
                      xhr = self.resnet2( xhr )
                      
                      # final fusion                       
                      x = self.convF1( torch.cat((xlr,xhr),1) )
                      x = self.convF2( F.relu(x) )
        ...
        ## train Conv-AE
            ...
            if 1*1:
                ...
                lrUpdate       = [1e-3,1e-4,1e-4,1e-5,1e-5,1e-6,1e-7]
            ...
            fileAEModelInit = './ResSSTNATL60/model_patchDataset_NATL60withMETOP_SST_128_512_011WFilter005_NFilter100_RU003_Torch_Alpha101_AETRwoTrueData07D20N00W00_Nproj10_Grad_00_01_00_modelAEGradFP_iter040.mod'            
...
#                            genSuffixModel = genSuffixModel+'_Std%03d'%(100*stdMask)
        ...
        ## train Conv-AE using test data with missing data
            ...
            fileAEModelInit = './ResSSTNATL60/model_patchDataset_NATL60withMETOP_SST_128_512_011WFilter005_NFilter100_RU003_Torch_Alpha101_AETRwoTrueData07D20N00W00_Nproj01_Grad_00_01_10_modelAEGradFP_iter015.mod'
            fileAEModelInit = './ResSSTNATL60/model_patchDataset_NATL60withMETOP_SST_128_512_011WFilter005_NFilter100_RU003_Torch_Alpha101_AETRwoTrueData07D20N00W00_Nproj10_Grad_00_01_00_modelAEGradFP_iter040.mod'
            fileAEModelInit = './ResSSTNATL60/model_patchDataset_NATL60withMETOP_SST_128_512_011WFilter005_NFilter100_RU003_Torch_Alpha101_AETRwithGTTrandTt07D20N00W00_Nproj01_Grad_00_01_10_modelAEGradFP_iter012.mod'
            #'./MNIST/mnist_DINConvAE_AE02N30W02_Nproj05_Encoder_iter006.mod'#'./MNIST/mnist_DINConvAE_AE02N30W02_Nproj10_Encoder_iter011.mod'
