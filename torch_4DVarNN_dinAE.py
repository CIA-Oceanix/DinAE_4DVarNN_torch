...
class model_GradUpdate1(torch.nn.Module):
    ...
    def forward(self, x,xpred,xobs,mask,grad_old):
...
class model_GradUpdate2(torch.nn.Module):
    ...
    def forward(self, x,xpred,xobs,mask,hidden,cell):
    ...
    def forward(self, x_inp,xobs,mask,g1=None,g2=None):
        ...
        if self.NGrad > 0:
            for kk in range(0,self.NGrad):
                ...
                # gradient update
                if self.OptimType == 0:
                    grad  = self.model_Grad( x, xpred, xobs, mask )
        
                elif self.OptimType == 1:               
                  if kk == 0:
                    grad  = self.model_Grad( x, xpred, xobs, mask, g1 )
                  else:
                    grad  = self.model_Grad( x, xpred, xobs, mask,grad_old )
                  grad_old = torch.mul(1.,grad)
        
                elif self.OptimType == 2:               
                  if kk == 0:
                    grad,hidden,cell  = self.model_Grad( x, xpred, xobs, mask, g1, g2 )
                  else:
                    grad,hidden,cell  = self.model_Grad( x, xpred, xobs, mask, hidden, cell )
